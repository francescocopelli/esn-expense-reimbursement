...
export default function ProjectExpenseFormClient({ projectId, projectName, categories }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    fd.append('event_name', eventName || projectName)
  ...
  if (success) {
    return (
      <div style={{ textAlign: 'center', padding: '2rem' }}>
        ...
        <h3>Rimborso inviato!</h3>
        <p style={{ color: '#6c757d' }}>La tua richiesta è stata inviata al supervisore del progetto.</p>
        <a href="/dashboard/my_reimbursement" className="btn btn-esn-cyan">Vai ai miei rimborsi</a>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="form-group">
        <label className="form-label" htmlFor="event_name">Descrizione attività *</label>
        <input id="event_name" type="text" required className="form-control"
          placeholder={`es. Attività nell'ambito di ${projectName}`}
            ...
            <div key={i} style={{
              ...
              <div className="form-group">
                ...
                <input type="text" required className="form-control"
                  placeholder="es. Materiali evento"
                ...
                <div className="form-group">
                  ...
                  {!maxOk && (
                    <p style={{ color: '#fd7e14', fontSize: '0.8rem', marginTop: '0.25rem' }}>
                      ⚠️ Supera il limite di €{cat?.max_amount?.toFixed(2)}
                    </p>
                  )}
                </div>
              </div>
      ...
      <button type="submit" disabled={loading} className="btn btn-esn-cyan w-full">
        {loading ? 'Invio in corso...' : '📤 Invia Rimborso al Supervisore'}
      </button>
    </form>
  )
}
